            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @foreach ($applicants as $app)
                        <form method="POST" enctype="multipart/form-data" action="{{ route('updateverificationapp', $app->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="my-2">
                                <x-input-label for="cnic" :value="__('CNIC')" />
                                <x-text-input id="cnic" class="block mt-1 w-full p-2" type="text" name="cnic" :value="old('cnic', $app->applicants->cnic)" required autofocus autocomplete="cnic" />
                                <x-input-error :messages="$errors->get('cnic')" class="mt-2" />
                            </div>
        
                            <div class="my-2">
                                <x-input-label for="name" :value="__('Name')" />
                                <x-text-input id="name" class="block mt-1 w-full p-2" type="text" name="name" :value="old('name', $app->applicants->name)" required autofocus autocomplete="name" />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>
    
                            <div class="my-2">
                                <x-input-label for="fathername" :value="__('Father/Husband Name')" />
                                <x-text-input id="fathername" class="block mt-1 w-full p-2" type="text" name="fathername" :value="old('fathername', $app->applicants->fathername)" required autofocus autocomplete="fathername" />
                            ...
                            <div class="my-2">
                                ...
                                <x-text-input id="scan_file" class="block mt-1 w-full p-2" type="file" name="scan_file" :value="old('scan_file')" autofocus autocomplete="scan_file" />
                            ...
                            <div class="mt-2">
                                <p>Note: if you want to update cnic in your application and father name in your child application, then first update your profile and then submit this form.</p>
                            </div>
                        </form>
                    @endforeach
                    @if ($errors->any())
